...
func main() {
	...
	// 4. 订阅踢下线事件（原始处理器）
	fmt.Println("\n=== 订阅踢下线事件 ===")
	err = client.SubscribeKickOff(openId, func(payload string) {
	...
	// 5. 订阅踢下线事件（类型化处理器）
	err = client.SubscribeKickOffTyped(openId, func(event notification.UserKickOffEvent) {
	...
	// 6. 订阅登录事件（类型化处理器）
	fmt.Println("\n=== 订阅登录事件 ===")
	err = client.SubscribeLoginTyped(openId, func(event notification.UserLoginEvent) {
	...
	// 7. 订阅退出事件（类型化处理器）
	fmt.Println("\n=== 订阅退出事件 ===")
	err = client.SubscribeLogoutTyped(openId, func(event notification.UserLogoutEvent) {
